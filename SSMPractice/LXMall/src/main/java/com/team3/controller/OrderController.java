...
//		}
	...
	public String add(Order order, Model model, HttpServletRequest request) throws UnsupportedEncodingException {
		//生成订单
		System.out.println("这是订单编号");
		System.out.println("账号：" + ToolUtil.getCookieno(request));
		System.out.println("收货人编号：" + str_consigeneenumber);
		System.out.println("订单时间：" + ThisSystemUtil.getSystemTime());
		System.out.println("订单总价：" + order.getOrderMoney());
	...
	public @ResponseBody String isSelectAddress(Goods goods) {
	...
	public String oneuserupdate(Order order, Model model, HttpServletRequest request) throws UnsupportedEncodingException {
		orderService.updateOrderStatus(order);
