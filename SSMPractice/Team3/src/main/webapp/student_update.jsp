...
<body>
	...
	<form id="student_update_form" action="${basePath}student/update.do">
		编号：<input type="text" name="id" value="${student.id}"
			readonly="readonly"><br> 姓名：<input type="text"
			name="name" value="${student.name}"><br> 密码：<input
			type="password" name="password" value="${student.password}"><br>
		班级： <select name="clazz.id">
			<c:forEach items="${clazzes}" var="c">
				<option value="${c.id}" ${student.clazz.id==c.id?'selected':'' }>${c.name}</option>
			</c:forEach>
		</select><br> 头像:<input type="file" name="file" onchange="uploadImg()"><br>
		<input type="hidden" name="url" id="imgurl"> 预览：<img id="pre"
			style="border: 1px solid red; width: 100px; height: 150px; object-fit: contain"
			src="${student.url}"></img><br> 详情：
		<textarea rows="" cols="" name="desc" class="ckeditor">${student.desc}</textarea>
	...
	<script type="text/javascript">
		$(function() {
			$("#name").blur(function() {
				//console.log("blur");
				$.post("${basePath}student/validate.do", {
					name : $(this).val()
				}, function(data) {
					//$("#nameerr").html(data);
					console.log(data);
